...
| 💎 **提示词工具** | AI 智能润色 · AI 风格预设生成 · 多风格预设 · 批量出图 |
| 🏪 **用户中心** | 注册登录 · API Key 管理 · 用量仪表盘 · 月配额进度 · 每日签到 |
...
| 模块 | 能力 |
|---|---|
| 📊 **数据看板** | 用户 · 订单 · 生成量 · 收入 · 转化率 · ARPU · 令牌用量分布 |
| 📡 **系统监控** | SSE 实时推送 QPS · 性能 · 号池 · 定时器 |
| 📜 **调用日志** | 实时流 + 历史分页 · **Web/API 来源区分** · 调用详情 |
| 🛡️ **风险评分** | 四维加权 · **全参数可调** · 阶梯封禁 · 防误封默认 · **AI 智能研判（可选）** |
| 💰 **配额防护** | 月配额防二次分发 · 撞额降速 · 撞额名单 · 单 Key 多 IP 告警 |
| 🤖 **号池管理** | 批量导入 · 健康监控 · 异常清理 · 智能补号 · 注册机 |
| 📣 **公告管理** | Banner + 弹窗双模式 · 类型 · 优先级 · 时间窗 |
| ⚙️ **系统设置** | 存储模式 · 调度并发 · 限速 · 缓存 · 风险阈值 · 月配额（热更新） |
| 🗑️ **内容治理** | 图片瀑布管理 · 完整提示词查看 · 分享审核 · 批量删除 |
...
- **AI 智能研判（可选，默认关）**：管理员可按需让大模型基于用户多维画像（账号年龄/付费/同 IP 关联/实时信号/行为）评估风险，给出分数、理由与建议——**仅供人工参考，不自动处置**。复用站点已配模型，无需额外基础设施。

---

## 💰 配额防护（防二次分发）

订阅制下防止用户搭中转站转卖 API 的双层机制：**令牌桶管限速 + 月配额管总量**。

- **月配额**：套餐级「每月令牌上限」（0=不限），按自然月计量、月初自动重置（Redis 计数零重置任务）。
- **撞额降速**（默认关，先观测）：用户当月令牌用量撞顶后，令牌桶恢复速率被砍低——转卖产能归零，正常用户温和退化（画质与单张速度不受影响）。支持**固定速率**与**按套餐比例**两种降速模式。
- **观测工具**：数据统计页「令牌用量分布」(P50–P99) 辅助定配额；「配额告警」页汇总**撞额名单** + **单 Key 多 IP 告警**（疑似中转站特征），仅展示供人工研判，不自动封号。

> 设计取舍：目标是「让转卖无利可图 + 快速发现」，而非「100% 杜绝」。撞额处置默认关闭，建议先观测名单、确认无误伤再开启。
...
# 3. 设置超级管理员：用 .env.prod 里 SUPERADMIN_EMAIL 指定的邮箱注册一个账号，
#    登录后即自动获得最高权限（改超管只需改该值并重启后端）。
```
...
| `GET` | `/api/admin/risk/scores` · `/detail` · `/profile` | 风险评分 / 用户详情 / 画像聚合 |
| `POST` | `/api/admin/risk/unban` · `/ai-analyze` | 解封 / AI 风险研判 |
| `GET` | `/api/admin/quota/alerts` | 撞额名单 + 多 IP 告警 |
