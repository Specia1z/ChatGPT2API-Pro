...
func scanAnnouncements(rows *sql.Rows) ([]model.Announcement, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&a.ID, &a.Title, &a.Content, &a.Type, &a.Link, &a.Priority, &a.Enabled, &start, &end, &created); err != nil {
			return nil, err
		}
...
func (s *MySQLStore) ListActiveAnnouncements() ([]model.Announcement, error) {
	rows, err := s.db.Query(`SELECT id, title, COALESCE(content,''), type, COALESCE(link,''), priority, enabled,
...
func (s *MySQLStore) CreateAnnouncement(a *model.Announcement) (int64, error) {
	res, err := s.db.Exec(`INSERT INTO announcements (title, content, type, link, priority, enabled, start_at, end_at)
		VALUES (?,?,?,?,?,?,?,?)`,
		a.Title, a.Content, a.Type, a.Link, a.Priority, a.Enabled, nullableTime(a.StartAt), nullableTime(a.EndAt))
	if err != nil {
		return 0, err
	}
...
func (s *MySQLStore) UpdateAnnouncement(a *model.Announcement) error {
	_, err := s.db.Exec(`UPDATE announcements SET title=?, content=?, type=?, link=?, priority=?, enabled=?, start_at=?, end_at=? WHERE id=?`,
		a.Title, a.Content, a.Type, a.Link, a.Priority, a.Enabled, nullableTime(a.StartAt), nullableTime(a.EndAt), a.ID)
	return err
}
