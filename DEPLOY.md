...
- [七、临时邮箱服务（注册机用，可选）](#七临时邮箱服务注册机用可选)
- [八、升级 / 回滚 / 备份](#八升级--回滚--备份)
- [九、常见问题排查](#九常见问题排查)
...
后台「注册机」自动批量注册上游账号时，需要一个能收验证码的临时邮箱服务。本项目内置的 `cloudflare_temp_email` 邮件提供商，对接的是**独立部署的开源项目**：

> **上游项目**：[dreamhunter2333/cloudflare_temp_email](https://github.com/dreamhunter2333/cloudflare_temp_email)
> 基于 Cloudflare Email Routing + Workers + D1，自建一套临时邮箱服务（自有域名收件、API 创建邮箱、拉取邮件）。

> [!NOTE]
> 这是**与本项目独立的外部服务**，需你自行在 Cloudflare 上部署。本项目只作为它的 API 客户端调用——不部署它也不影响生图/支付等核心功能，仅注册机的自动收码不可用。

### 部署上游服务（简要）

按 [cloudflare_temp_email 官方文档](https://github.com/dreamhunter2333/cloudflare_temp_email) 操作，大致流程：

1. 一个托管在 Cloudflare 的域名，开启 **Email Routing**
2. 部署该项目的 Worker（后端 API）+ D1 数据库（存邮件）
3. 配置 `ADMIN_PASSWORD`（管理密码）、收件域名
4. 记下：**API 地址**、**管理密码**、**可用域名**

### 在本项目后台配置

后台 → **系统设置 → 邮件**，添加 `cloudflare_temp_email` 提供商，对应字段：

| 后台字段 | 说明 | 对应上游 |
|---|---|---|
| `api_base` | 上游服务 API 地址，如 `https://mail.your-domain.com` | Worker 部署地址 |
| `admin_password` | 创建邮箱用的管理密码 | 该项目的 `ADMIN_PASSWORD` |
| `domain` | 可用收件域名（可多个，轮询使用） | Email Routing 配置的域名 |

配置后，本项目通过 `POST /admin/new_address`（带 `x-admin-auth` 管理密码头）创建邮箱、`GET /api/mails`（带 JWT）拉取验证码——这些是 cloudflare_temp_email 的标准 API，无需额外适配。

> 验证码识别已内置：自动从邮件 HTML/文本中提取 6 位数字验证码，支持「验证码 / verification code / code is」等多种关键词与背景色匹配。
...
<div align="center">
<sub>部署遇到问题，先看「九、常见问题排查」，多数坑都在 Nginx 反代的 X-Real-IP 与 SSE 配置上。</sub>
</div>
